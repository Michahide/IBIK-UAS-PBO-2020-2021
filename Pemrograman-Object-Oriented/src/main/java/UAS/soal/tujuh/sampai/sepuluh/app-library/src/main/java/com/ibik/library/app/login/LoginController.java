...
import com.ibik.library.app.dashboard.ProfileController;
...
public class LoginController {
	...
	private void initialize() {
		...
		labelIcon.setIcon(new ImageIcon(
				"C:\\Users\\mervi\\OneDrive\\Dokumen\\GitHub\\IBIK-UAS-PBO-2020-2021\\Pemrograman-Object-Oriented\\src\\main\\java\\UAS\\soal\\tujuh\\sampai\\sepuluh\\app-library\\src\\main\\java\\com\\ibik\\library\\images\\logo-ibik-web.png")); //Ubah lokasi sesuai direktori yang ada
		...
		btnLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(callback.getResponce()) {
					frameMain.dispose();
					new ProfileController(user).setVisible(true);  //set window bisa dilihat karena nilainya = true
				}else {
					JOptionPane.showMessageDialog(null, callback.getLabel(),"Error",JOptionPane.ERROR_MESSAGE); //Ubah result.getLabel menjadi callback.getLabel
				}				
			}
		});		
		panel.add(btnLogin);
	...
	public class CallbackResultSet extends LoginController { //Tambahkan class CallbackResultSet
		//Tambahkan atribut dan method sesuai class diagram
	...
	public CallbackResultSet ValidationForm(String username, String password) { //sending parameter username & password
		...
		//sending username and password to DAO
			...
			Integer NIK = Integer.parseInt(username); //Ubah ke Integer seusai class diagram
			users.setNIK( NIK );
